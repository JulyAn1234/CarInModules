...
def make_request(event=None):
    ...
    if re.match(r"[A-Z]\d+", data):  # Check if the response matches the format LetterNumber
        message = f"Listo! Tu estacionamiento es: {data}. Tenga un buen día!"
        show_custom_dialog("Response", message, 8000, entry)
    else:
        message = data
...
root = tk.Tk()
root.title("Módulo de entrada al estacionamiento")  # Add the title
...
# Background Image
bg_image = Image.open("ITT.jpg")
bg_image = bg_image.resize((root.winfo_screenwidth(), root.winfo_screenheight()))
bg_photo = ImageTk.PhotoImage(bg_image)
bg_label = tk.Label(root, image=bg_photo)
bg_label.place(x=0, y=0, relwidth=1, relheight=1)

title_label = tk.Label(root, text="Módulo de entrada al estacionamiento", bg="white", fg="#333333", font=("Arial", 24, "bold"))  # Adjust the font size and style
